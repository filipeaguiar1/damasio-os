...
async function compactDemoPhoto(file:File){return new Promise<string>((resolve,reject)=>{const image=new Image();const source=URL.createObjectURL(file);image.onload=()=>{const scale=Math.min(1,900/Math.max(image.width,image.height));const canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.round(image.width*scale));canvas.height=Math.max(1,Math.round(image.height*scale));canvas.getContext("2d")?.drawImage(image,0,0,canvas.width,canvas.height);URL.revokeObjectURL(source);resolve(canvas.toDataURL("image/jpeg",.68))};image.onerror=()=>{URL.revokeObjectURL(source);reject(new Error("The selected photo could not be read."))};image.src=source})}

export async function uploadPropertyProfilePhoto(propertyId:string,file:File){
 if(readDemoSession()||!isSupabaseConfigured()){const url=await compactDemoPhoto(file);if(getLead(propertyId)){try{setPropertyPhoto(propertyId,url)}catch(error){if(!(error instanceof DOMException&&error.name==="QuotaExceededError"))throw error;return URL.createObjectURL(file)}}return url}
 const supabase=getSupabaseBrowserClient();
 const{data}=await supabase.auth.getSession();
 const token=data.session?.access_token;
 if(!token)throw new Error("Your session expired. Sign in again.");
 const form=new FormData();form.append("file",file);
 const response=await fetch("/api/customer/property/photo",{method:"POST",headers:{authorization:`Bearer ${token}`},body:form});
 const result=await response.json();
 if(!response.ok||!result.url)throw new Error(result.error||"Property photo could not be saved.");
