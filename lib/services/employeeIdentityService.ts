...
export type EmployeeOperationalIdentity = { name: string; crew: string; employeeId?: string | null; crewId?: string | null };
...
export async function loadEmployeeOperationalIdentity(): Promise<EmployeeOperationalIdentity> {
  ...
  const fallback = { name: local.name || "Employee", crew: local.crew || "Crew A", employeeId: null, crewId: null };
  ...
  try {
    ...
    const select = "id,full_name,email,crew_id,profile_id,crews(id,name)";
