-- Customer wallet credits: 1 credit = 1 CAD dollar.
-- Balances are stored in cents and may only be mutated by trusted server code.
begin;
...
create unique index if not exists customer_wallet_transactions_stripe_intent_unique
  on public.customer_wallet_transactions(stripe_payment_intent_id)
  where stripe_payment_intent_id is not null;

create index if not exists customer_wallet_transactions_customer_created_idx
  on public.customer_wallet_transactions(company_id, customer_id, created_at desc);
...
create policy customer_wallet_read_own
...
using (
  exists (
    ...
    where c.id = customer_wallets.customer_id
      and c.company_id = customer_wallets.company_id
      and c.profile_id = auth.uid()
      and c.archived_at is null
  )
);

create policy customer_wallet_transactions_read_own
...
using (
  exists (
    ...
    where c.id = customer_wallet_transactions.customer_id
      and c.company_id = customer_wallet_transactions.company_id
      and c.profile_id = auth.uid()
      and c.archived_at is null
  )
);
...
begin
  ...
  if not exists (
    ...
    where id = p_customer_id and company_id = p_company_id and archived_at is null
  ) then
  ...
  if v_existing.id is not null then
    return query select v_existing.balance_after_cents, false;
    return;
  end if;
  ...
  insert into public.customer_wallet_transactions(
    ...
    balance_after_cents, stripe_payment_intent_id, description
  ) values (
    v_wallet.id, p_company_id, p_customer_id, 'topup', p_amount_cents,
    v_wallet.balance_cents, p_stripe_payment_intent_id, nullif(trim(coalesce(p_description, '')), '')
  );
