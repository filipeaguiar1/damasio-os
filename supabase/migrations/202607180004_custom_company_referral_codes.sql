-- Company Admins may customize their public referral code.
begin;
...
begin
  loop
    ...
    exit when not exists(select 1 from public.organizations where referral_code=v_code);
  end loop;
  return v_code;
end
$$;
...
begin
  if nullif(trim(coalesce(new.referral_code,'')),'') is null then
    new.referral_code:=public.generate_company_referral_code();
  else
...
create trigger organizations_referral_code_default
before insert or update of referral_code on public.organizations
for each row execute function public.set_company_referral_code();

do $$
declare v_id uuid;
begin
  for v_id in select id from public.organizations where referral_code is null loop
    update public.organizations
    set referral_code=public.generate_company_referral_code()
    where id=v_id;
  end loop;
end
$$;
...
create unique index if not exists organizations_referral_code_unique
...
alter table public.organizations add constraint organizations_referral_code_format
check(referral_code~'^[A-Z0-9]{4,12}$');
