...
begin
  ...
  if not exists (
    ...
    where e.id = p_employee_id
      and e.crew_id = p_crew_id
      and e.active
      and coalesce(e.company_id, e.organization_id) = v_company_id
  ) then
    raise exception 'Destination Employee is not active in this company.';
  end if;

  select coalesce(array_agg(distinct u.job_id), '{}'::uuid[])
  into v_job_ids
  from unnest(coalesce(p_job_ids, '{}'::uuid[])) as u(job_id);

  if cardinality(v_job_ids) = 0 then
    raise exception 'Select at least one Customer Job.';
  end if;

  if exists (
    select 1
    from unnest(v_job_ids) as u(job_id)
    left join public.jobs j on j.id = u.job_id
    where j.id is null
       or not j.active
       or coalesce(j.company_id, j.organization_id) <> v_company_id
  ) then
    raise exception 'One or more Jobs are unavailable in this company.';
  end if;
  ...
  where rs.visit_id = v.id
    and v.id = any(v_visit_ids)
    and coalesce(v.company_id, v.organization_id) = v_company_id;
  ...
  return jsonb_build_object(
    ...
    'futureVisitsReassigned', v_moved_visits,
