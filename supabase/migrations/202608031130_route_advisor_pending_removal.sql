...
create table if not exists public.visit_route_removal_audit (
  ...
  previous_crew_id uuid references public.crews(id) on delete set null,
  previous_route_order integer,
  scheduled_date date not null,
  created_at timestamptz not null default now()
);

create index if not exists visit_route_removal_audit_company_idx
  on public.visit_route_removal_audit(company_id, created_at desc);
create index if not exists visit_route_removal_audit_visit_idx
  on public.visit_route_removal_audit(visit_id, created_at desc);

alter table public.visit_route_removal_audit enable row level security;
grant select on public.visit_route_removal_audit to authenticated;
grant all on public.visit_route_removal_audit to service_role;

drop policy if exists visit_route_removal_audit_company_read
on public.visit_route_removal_audit;
create policy visit_route_removal_audit_company_read
on public.visit_route_removal_audit
