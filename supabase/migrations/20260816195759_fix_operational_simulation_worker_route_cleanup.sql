-- Fix exact simulation purge so partially-cleaned runs cannot leave Workers, Crews or Routes behind.
-- Simulation Employees can intentionally have no profile_id; match them by the same protected
-- email marker used by the V1/V2 purge wrappers. Routes are also collected from proven
-- simulation Crews so cleanup does not depend on Visits still existing.
create or replace function public._purge_operational_simulation_public_graph(p_company_id uuid, p_email_pattern text)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  where coalesce(e.company_id, e.organization_id) = p_company_id
    and (
      e.profile_id = any(v_profile_ids)
      or lower(coalesce(e.email, '')) like lower(p_email_pattern)
    );
  ...
  if cardinality(v_customer_ids) = 0
     and cardinality(v_profile_ids) = 0
     and cardinality(v_employee_ids) = 0 then
  ...
  if cardinality(v_employee_ids) > 0 and exists (
    select 1 from public.visit_assignment_audit a
  ...
  if cardinality(v_profile_ids) > 0 and exists (
    select 1 from public.visit_reopen_events r
  ...
  where visit_id = any(v_visit_ids) or job_id = any(v_job_ids) or to_employee_id = any(v_employee_ids) or to_crew_id = any(v_crew_ids);
  delete from public.visit_billing_events
  where visit_id = any(v_visit_ids) or customer_id = any(v_customer_ids) or property_id = any(v_property_ids) or job_id = any(v_job_ids);
  ...
  where customer_id = any(v_customer_ids) or property_id = any(v_property_ids) or job_id = any(v_job_ids);
  ...
  v_employee_count := cardinality(v_employee_ids);
  delete from public.visits where id = any(v_visit_ids) and coalesce(company_id, organization_id) = p_company_id;
  delete from public.routes where id = any(v_route_ids) and coalesce(company_id, organization_id) = p_company_id;
  delete from public.jobs where id = any(v_job_ids) and coalesce(company_id, organization_id) = p_company_id;
  delete from public.quotes where customer_id = any(v_customer_ids) and coalesce(company_id, organization_id) = p_company_id;
  delete from public.properties where id = any(v_property_ids) and coalesce(company_id, organization_id) = p_company_id;

  v_customer_count := cardinality(v_customer_ids);
  delete from public.customers where id = any(v_customer_ids) and coalesce(company_id, organization_id) = p_company_id;
  delete from public.employees where id = any(v_employee_ids) and coalesce(company_id, organization_id) = p_company_id;
  ...
  delete from public.profiles where id = any(v_profile_ids) and coalesce(company_id, organization_id) = p_company_id;
