...
-- Compatibility repair for databases where public.visits does not have
-- updated_at. This keeps route_stops as the source of truth and still projects
-- route_order back to visits for existing Admin/Worker screens.
...
begin
  ...
  -- Projection only: visits.route_order remains for old readers, but route_stops
  -- is the official order. Do not reference visits.updated_at; older databases
  -- do not have that column.
...
begin
  ...
  v_result := public.replace_canonical_route_order_v2(
    p_route_id,
    p_ordered_visit_ids,
    coalesce(nullif(trim(p_source), ''), 'employee_smart_route'),
    v_profile.id,
    p_expected_version,
    false
  );
...
grant execute on function public.replace_canonical_route_order_v2(
  uuid, uuid[], text, uuid, integer, boolean
) to service_role;
