-- Etapa 7: harden mobile role permission boundaries.
-- Managers must have at least one operational read/manage permission before
-- reading the daily operations dashboard through the SECURITY DEFINER RPC.
    ...
    where id=auth.uid()
      ...
      and (
        ...
        or (
          role::text='manager'
          and (
            coalesce(manager_permissions->>'customers','none') in ('read','manage')
            or coalesce(manager_permissions->>'properties','none') in ('read','manage')
            or coalesce(manager_permissions->>'schedule','none') in ('read','manage')
            or coalesce(manager_permissions->>'dispatch','none') in ('read','manage')
            or coalesce(manager_permissions->>'routes','none') in ('read','manage')
            or coalesce(manager_permissions->>'tasks','none') in ('read','manage')
            or coalesce(manager_permissions->>'reports','none') in ('read','manage')
          )
        )
      )
  ), selected_day as (
