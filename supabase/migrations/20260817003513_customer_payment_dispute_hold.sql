...
create index if not exists service_requests_payment_dispute_lookup_idx
  on public.service_requests (customer_id, payment_id, status)
  where request_type = 'payment_dispute';

create unique index if not exists service_requests_one_open_payment_dispute_idx
  on public.service_requests (customer_id, payment_id)
  where request_type = 'payment_dispute'
    and status in ('pending','open','investigating');
...
begin
  if length(v_reason) < 5 then
    raise exception 'Please explain the payment issue in at least 5 characters.';
  end if;
  ...
  if not found then
    raise exception 'Completed Visit was not found for this Customer.';
  end if;
  ...
  if not found then
    raise exception 'No paid invoice is linked to this completed Visit.';
  end if;
  ...
  if not found then
    raise exception 'No successful payment is linked to this invoice.';
  end if;
  ...
  if v_existing_id is not null then
    select contact_email into v_support_email
    ...
    return jsonb_build_object(
      ...
      'supportEmail', v_support_email
    );
  end if;

  insert into public.service_requests (
    organization_id, company_id, customer_id, property_id,
    service_name, message, status, request_type,
    visit_id, invoice_id, payment_id
  ) values (
    ...
    v_reason, 'pending', 'payment_dispute',
    v_visit.id, v_invoice.id, v_payment.id
  )
  ...
  if found then
    if v_payout.status not in ('transferred','refunded','chargeback_reversed') then
      ...
      update public.company_payout_items
      set hold_reason = 'Customer payment dispute opened after payout/reversal · request ' || v_request_id::text,
          updated_at = now()
      where id = v_payout.id;
    end if;
  end if;
