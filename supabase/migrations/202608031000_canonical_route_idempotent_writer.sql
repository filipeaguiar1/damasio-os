-- Canonical Route idempotent writer
-- route_stops is the only durable source of route membership and order.
-- Reordering no longer updates visits.route_order, avoiding legacy Visit triggers
-- that recreate route_stops and violate route_stops_route_id_visit_id_key.

begin;

-- Remove only the recursive triggers introduced by the temporary compatibility patch.
...
begin
  ...
  left join public.route_stops s
    on s.route_id = v.route_id and s.visit_id = v.id
  where v.route_id = p_route_id
  ...
  insert into public.route_order_state(
    route_id, company_id, version, last_source, last_actor_profile_id, updated_at
  ) values (
    p_route_id, v_company_id, 1, 'initialization', v_profile.id, now()
  ...
  -- Move current positions out of the canonical range first. This avoids the
  -- unique(route_id, position) constraint while preserving each existing row.
  ...
  if v_has_property_id then
    insert into public.route_stops(
      company_id, organization_id, route_id, visit_id, property_id, position, updated_at
    )
    select
      v_company_id,
      v_company_id,
    ...
    insert into public.route_stops(
      company_id, organization_id, route_id, visit_id, position, updated_at
    )
    select
      v_company_id,
      v_company_id,
  ...
  where s.route_id = p_route_id
    and not (s.visit_id = any(v_requested));
  ...
  insert into public.employee_smart_route_state(
    company_id, route_id, crew_id, route_date,
    original_order, applied_order,
    origin_label, origin_latitude, origin_longitude,
    active, applied_by_profile_id, applied_at,
    route_version, updated_at
  ) values (
    ...
    v_requested,
    coalesce(p_origin_label, ''), p_origin_latitude, p_origin_longitude,
  ...
  insert into public.route_order_audit(
    company_id, route_id, actor_profile_id, source,
    previous_order, next_order, route_version
  ) values (
    v_company_id, p_route_id, v_profile.id,
    coalesce(nullif(trim(p_source), ''), 'route_order_update'),
    v_previous, v_requested, v_next_version
  );
  ...
  begin
    perform public.queue_route_map_rebuild(p_route_id, v_company_id, 'canonical_route_idempotent_writer');
