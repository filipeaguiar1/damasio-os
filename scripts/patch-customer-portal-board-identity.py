...
request_old = '''export async function createCustomerPortalRequest(input: { serviceName: string; message?: string }) {
...
request_new = '''async function getCustomerFallbackIdentity(supabase: any, board: CustomerPortalBoard) {
  if (!board.property?.propertyId || !board.property.customerId) {
    throw new Error("Customer property not found for this account.");
  }
...
feedback_old = '''export async function submitCustomerPortalFeedback(input: { visitId?: string; taskId?: string; rating: number; comment?: string }) {
  const supabase = getSupabaseBrowserClient() as any;
  const args = {
...
for old, new in ((request_old, request_new), (feedback_old, feedback_new)):
    if old in text:
