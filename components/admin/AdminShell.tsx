...
function mobileAdminPath(pathname: string) {
  const section = pathname.split("/").filter(Boolean)[1] || "";
  const supported = new Set(["command", "routes", "schedule", "customers", "tasks", "alerts", "estimates", "invoices", "requests", "employees", "finance", "reports"]);
  return section && supported.has(section) ? `/mobile/admin/${section}` : "/mobile/admin";
}
...
export function AdminShell({ children, active }: { children: React.ReactNode; active: string }) {
  ...
  useEffect(() => {
    if (window.matchMedia("(max-width: 900px)").matches) {
      router.replace(mobileAdminPath(pathname));
      return;
    }
    ...
    <div className="studio-shell">
      <header className="studio-topnav">
        ...
        <nav className="studio-nav" aria-label="Admin navigation">
          {topNav.map(([label, href]) => <Link key={href} href={href} className={active === label || (label === "Dispatch & Routes" && active === "Routes") || (label === "Operations Studio" && active === "Dashboard") ? "active" : ""}>{label}</Link>)}
        </nav>
      ...
      <footer className="studio-bottom-status"><span><i></i> Live sync active</span><span>{pendingRequests} approvals waiting</span><span>{unread} unread alerts</span><Link href="/admin/finance">Finance queue</Link></footer>
      <nav className="mobile-shell-bottom" aria-label="Admin subpage navigation"><Link href="/mobile/admin"><i>H</i><span>Home</span></Link><Link href="/mobile/admin/schedule"><i>S</i><span>Schedule</span></Link><Link href="/mobile/admin/routes"><i>R</i><span>Routes</span></Link><Link href="/mobile/admin/tasks"><i>!</i><span>Tasks</span></Link><button type="button" onClick={() => setMobileMenuOpen(true)}><i>...</i><span>More</span></button></nav>
    </div>
  );
}
