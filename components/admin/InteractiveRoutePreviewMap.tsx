...
export function InteractiveRoutePreviewMap({ route, origin, capacity, lockedJobIds = [], onRemove }: Props) {
  ...
  const locked = useMemo(() => new Set(lockedJobIds), [lockedJobIds.join("|")]);
  ...
  useEffect(() => {
    ...
    if (coordinates.length < 2) {
      setGeometry(null);
      setStatus(route.length ? "Add a valid route start to draw the driving line." : "Select houses to preview the route.");
    ...
    void fetch("/api/map/route", {
      ...
      return await response.json() as { geometry: RouteLineString };
    }).then(result => {
      if (!cancelled) {
        setGeometry(result.geometry);
        setStatus("Click an unlocked numbered house to remove it from the preview.");
      }
    }).catch(() => {
      if (!cancelled) {
        setGeometry(null);
        setStatus("Houses mapped. Driving line is temporarily unavailable.");
      }
    });
    return () => { cancelled = true; };
  }, [routeKey, originKey]);
  ...
  useEffect(() => {
    ...
    const setup = () => {
      ...
      route.forEach((home, index) => {
        ...
        const title = isLocked ? `${home.name} is completed or active and cannot be removed` : `Remove ${home.name}`;
        const icon = L.divIcon({
          className: "advisor-marker-shell",
          html: `<button class="advisor-stop ${isLocked ? "locked" : ""}" title="${title}">${index + 1}</button>`,
          iconSize: [38, 38],
          iconAnchor: [19, 19],
        });
        const marker = L.marker([Number(home.latitude), Number(home.longitude)], { icon })
          .bindTooltip(`${index + 1}. ${home.name} · ${isLocked ? "locked" : "click to remove"}`);
  ...
  return <section className="advisor-preview-map">
    <div className="advisor-preview-status">
      <strong>{route.length}/{safeCapacity} houses</strong>
      <span>{status}</span>
    </div>
    <div ref={node} className="advisor-preview-leaflet" aria-label="Clickable route preview" />
    <style jsx global>{`
      ...
      .advisor-preview-status span{color:#64748b;font-size:12px}
      .advisor-preview-leaflet{height:560px;min-height:420px}
      ...
      .advisor-stop{background:#2563eb;color:#fff;cursor:pointer}
      .advisor-stop:hover{background:#dc2626;transform:scale(1.08)}
      ...
      @media(max-width:800px){.advisor-preview-leaflet{height:430px}.advisor-preview-status{align-items:flex-start;flex-direction:column}}
    `}</style>
  </section>;
}
