...
export function PortalShell({ children, active, type }: { children: React.ReactNode; active: string; type: "Customer" | "Employee" }) {
  ...
  return <div className={`admin-pro-shell portal-pro-shell ${type === "Employee" ? "employee-portal-shell" : "customer-portal-shell"}`}>
    <aside className={`pro-sidebar ${mobileMenuOpen ? "mobile-menu-open" : ""}`}><button type="button" className="mobile-menu-close" onClick={() => setMobileMenuOpen(false)} aria-label="Close menu">x</button><Link href={base} className="season-logo portal-brand-card" aria-label="4Ever Seasons portal"><div className="season-title"><span>4EVER</span><strong>SEASONS</strong></div><div className="grass-mask" aria-hidden="true"><span></span><span></span><span></span></div><div className="mower-man" aria-hidden="true"><i className="head"></i><i className="body"></i><i className="leg one"></i><i className="leg two"></i><i className="arm"></i><i className="mower"></i></div></Link><Link href={`${base}/profile`} className="admin-profile clickable-profile"><div className="profile-avatar">{avatar}</div><div><strong>{profileName}</strong><span>{subtitle}</span></div><b>v</b></Link><nav className="pro-nav portal-word-nav">{links.map(([label, href]) => <Link key={href} href={href} onClick={() => setMobileMenuOpen(false)} className={active === label ? "active" : ""}>{label}</Link>)}{type === "Customer" && <Link href="/" onClick={() => setMobileMenuOpen(false)}>Website</Link>}</nav><Link href={type === "Customer" ? "/customer/requests" : "/employee/training"} className="help-card"><span>?</span><div><strong>Need Help?</strong><small>Contact Support</small></div></Link><button type="button" className="mobile-menu-signout" onClick={() => void signOutAccount("/mobile/login")}>Sign out</button></aside>
