...
export function PortalShell({ children, active, type }: { children: React.ReactNode; active: string; type: "Customer" | "Employee" }) {
  ...
  const links = type === "Customer"
    ? [
        ["Dashboard", base, "H"], ["Services", `${base}/services`, "S"], ["Service Issues", `${base}/tasks`, "!"],
        ["History", `${base}/history`, "R"], ["Estimates", `${base}/estimates`, "E"], ["Notifications", `${base}/notifications`, "N"],
        ["Invoices", `${base}/invoices`, "I"], ["Payments", `${base}/payments`, "$"], ["Requests", `${base}/requests`, "+"],
        ["Feedback", `${base}/feedback`, "*"], ["My Property", `${base}/property`, "M"], ["Profile", `${base}/profile`, "P"],
      ]
  ...
  const initials = type === "Customer" ? "CU" : "FD";
  ...
  return <div className={`admin-pro-shell portal-pro-shell ${type === "Employee" ? "employee-portal-shell" : "customer-portal-shell"}`}>
    <aside className={`pro-sidebar ${mobileMenuOpen ? "mobile-menu-open" : ""}`}>
      ...
      <Link href={base} className="season-logo" aria-label="4Ever Seasons portal"><div className="season-title"><span>4EVER</span><strong>SEASONS</strong></div><div className="grass-mask" aria-hidden="true"><span></span><span></span><span></span></div><div className="mower-man" aria-hidden="true"><i className="head"></i><i className="body"></i><i className="leg one"></i><i className="leg two"></i><i className="arm"></i><i className="mower"></i></div></Link>
      <Link href={`${base}/profile`} className="admin-profile clickable-profile"><div className="profile-avatar">{initials}</div><div><strong>{type === "Customer" ? "Customer" : "Filipe Damasio"}</strong><span>{subtitle}</span></div><b>v</b></Link>
