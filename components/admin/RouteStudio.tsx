...
export function RouteStudio() {
  ...
  return <section className="desktop-route-studio">
    ...
    {mode === "move" && <section className="route-simple-workspace">
      ...
      <div className="route-simple-grid"><aside><div className="route-simple-head"><strong>{selected.length} selected</strong><span>{movableSource.length} scheduled</span></div><RouteChoiceList items={movableSource} selected={selected} onToggle={toggle} />{sourceRoute.some(item => ["completed", "in_progress"].includes(item.canonicalVisitStatus || "")) && <p className="route-lock-note">Completed and active houses stay locked on the original executor and route.</p>}</aside><section className="route-simple-summary route-move-summary"><span>ASSIGNMENT MODE</span><h2>{targetEmployee?.name || "Choose destination"}</h2><div className="route-move-mode"><button type="button" className={moveMode === "temporary" ? "active" : ""} onClick={() => setMoveMode("temporary")}><strong>Temporary</strong><small>Move only this Visit. Regular Employee stays unchanged.</small></button><button type="button" className={moveMode === "permanent" ? "active" : ""} onClick={() => setMoveMode("permanent")}><strong>Permanent</strong><small>Change the regular Employee for this Job and all future Visits.</small></button></div><p>{moveMode === "temporary" ? "The selected Visit moves to the destination Employee for this date only. The Job keeps its regular Employee for future work." : "The Job's regular Employee changes to the destination Employee. Future Scheduled Visits follow automatically, while completed history stays unchanged."}</p><button className="btn btn-primary" disabled={busy || !targetEmployee || !selected.length} onClick={() => void moveSelected()}>{busy ? "Moving..." : `${moveMode === "temporary" ? "Move temporarily" : "Move permanently"} (${selected.length})`}</button></section></div>
    </section>}
