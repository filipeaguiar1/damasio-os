...
export default function FeedbackPage() {
  ...
  async function submit() {
    ...
    try {
      const reviewedVisitId = current.id;
      const updated = await addCustomerFeedback({ visitId: reviewedVisitId, rating, comment });
      setBoard(updated);
  ...
  return <PortalShell type="Customer" active="Feedback">
    <div className="feedback-experience-hero">
      <div><span>YOUR EXPERIENCE</span><h1>Review completed services</h1><p>Choose a service date, open its review, and close it again whenever you need.</p></div>
    ...
    {loading ? <div className="feedback-empty-state"><i>…</i><h2>Loading completed services</h2></div> : completed.length === 0 ? <div className="feedback-empty-state"><i>✓</i><h2>No completed service yet</h2><p>When a crew completes a service, it will appear here.</p></div> : <div className="feedback-experience-layout">
      <aside className="feedback-visit-list">
        ...
        <div>{completed.map((visit) => {
          ...
          return <button key={visit.id} type="button" aria-expanded={open} className={open ? "active" : ""} onClick={() => toggleVisit(visit)} style={{ alignItems: "center" }}>
            ...
            <em style={{ display: "flex", alignItems: "center", gap: 8 }}>{done ? "Reviewed" : "Review"}<b aria-hidden="true" style={{ display: "inline-block", fontSize: 18, transform: open ? "rotate(180deg)" : "rotate(0deg)", transition: "transform .2s ease" }}>⌄</b></em>
          </button>;
        })}</div>
      </aside>

      {!current ? <section className="feedback-review-card" style={{ minHeight: 180, display: "grid", placeItems: "center", textAlign: "center", padding: 28 }}>
        <div><div style={{ width: 48, height: 48, borderRadius: 16, display: "grid", placeItems: "center", margin: "0 auto 12px", background: "#f0fdf4", color: "#16a34a", fontSize: 24 }}>⌄</div><h2 style={{ margin: 0 }}>Open a completed service</h2><p style={{ margin: "8px 0 0", color: "#64748b" }}>Tap the arrow beside a date to review that visit.</p></div>
        ...
        <header className="feedback-service-head">
          ...
          <div><strong>{dateLabel(current.scheduledDate)}</strong><small>{current.crewName || "Service crew"}</small><button type="button" onClick={() => setSelected("")} aria-label="Close service review" style={{ marginTop: 10, border: 0, background: "transparent", fontSize: 22, cursor: "pointer" }}>⌃</button></div>
        </header>
