...
type Tab = "upcoming" | "history" | "payments";
...
export default function CustomerPayments() {
  ...
  const operationalDetail = activeAgreement
    ...
    : nextVisit
      ? "A visit exists, but the new billing agreement has not been created yet"
      : "No active schedule or billing agreement";
  const visibleMessage = portalError || wallet.message;
    ...
    <PortalShell type="Customer" active="Payments">
      <div className="pv-hero">
        <div>
          ...
          <p>Upcoming services, visit history, account balance, and payment activity in one secure place.</p>
        </div>
      ...
      <div className="pv-layout">
        <main className="pv-main">
          ...
          {tab === "history" && <section className="pv-panel">
            <header><div><span>Completed services</span><h2>Visit history</h2><p>Every visit stays connected to feedback, Tasks, and payment records.</p></div></header>
          ...
          {tab === "payments" && <>
            <section className="pv-panel">
              <header><div><span>Account balance</span><h2>Add credits</h2><p>Add real Canadian-dollar account credit securely through Stripe.</p></div><button className="billing-refresh" onClick={() => void wallet.reload()} disabled={wallet.loading}>{wallet.loading ? "Loading..." : "Refresh balance"}</button></header>
            ...
            <section className="pv-panel" style={{ marginTop: 16 }}>
              <header><div><span>Account activity</span><h2>Payment history</h2><p>Deposits, service payments, tips, refunds, and adjustments are recorded here.</p></div></header>
