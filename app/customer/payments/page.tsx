...
export default function CustomerPayments() {
  ...
  return (
    <PortalShell type="Customer" active="Payments">
      ...
      <section className="billing-panel">
        ...
        <div className="billing-invoice-list">
          <article>
            ...
            <div className="billing-invoice-total" style={{ alignItems: "stretch", gap: 12, minWidth: 320 }}>
              <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "flex-end", gap: 8 }}>
                {creditOptions.map((credits) => {
                  const selected = selectedCredits.includes(credits);
                  return <button key={credits} className={selected ? "btn btn-primary" : "btn btn-outline"} type="button" aria-pressed={selected} disabled={wallet.openingCredits > 0} onClick={() => toggleCredits(credits)}>{selected ? `✓ ${credits}` : `${credits}`} credits</button>;
                })}
                <button className={editingCredits ? "btn btn-primary" : "btn btn-outline"} type="button" aria-pressed={editingCredits} disabled={wallet.openingCredits > 0} onClick={() => setEditingCredits((value) => !value)}>Custom</button>
              </div>

              {editingCredits && <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 8 }}><input aria-label="Custom credit amount" type="number" min={5} max={1000} step={1} placeholder="Custom" value={customCredits} onChange={(event) => setCustomCredits(event.target.value)} style={{ width: 130 }} /><small>{customCredits && !validCustom ? "Enter 5–1,000" : "Custom amount"}</small></div>}

              <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "flex-end", alignItems: "center", gap: 16 }}>
      ...
      <section className="billing-help-strip"><div><i>i</i><span><strong>Stripe test mode</strong><small>Use Stripe&apos;s test card 4242 4242 4242 4242, any future expiry date and any CVC.</small></span></div><Link className="btn btn-outline" href="/customer/requests">Request help</Link></section>
    </PortalShell>
  );
}
