...
type Tab = "companies" | "leads" | "access" | "health" | "audit";
...
export default function MasterPage(){
  ...
  async function convertLead(lead:Lead){if(!lead.assigned_company_id){setMessage("Assign a company before accepting this lead.");return;}
    if(isDemo){setLeads(v=>v.map(x=>x.id===lead.id?{...x,status:"converted"}:x));setMembers(v=>[{id:uid("customer"),company_id:lead.assigned_company_id!,kind:"customer",name:lead.full_name,email:lead.email,active:true},...v]);addAudit("lead.converted",lead.assigned_company_id,{lead:lead.full_name,address:lead.address});setMessage(`${lead.full_name} was added to ${companyName(lead.assigned_company_id)} as a new customer. The company admin was notified.`);return;}
    const supabase=getSupabaseBrowserClient() as any;const {error}=await supabase.rpc("master_convert_lead_to_customer",{p_lead_id:lead.id});if(error){setMessage(error.message);return;}setLeads(v=>v.map(x=>x.id===lead.id?{...x,status:"converted"}:x));setMessage("Lead converted to Customer + Property and company notified.");
  }
  async function requestAccess(e:FormEvent<HTMLFormElement>){e.preventDefault();if(!accessCompany)return;const fd=new FormData(e.currentTarget);const code=String(Math.floor(100000+Math.random()*900000));const row:AccessRequest={id:uid("access"),company_id:accessCompany.id,access_level:String(fd.get("level")),delivery_channel:String(fd.get("channel")),request_reason:String(fd.get("reason")||""),code_expires_at:new Date(Date.now()+30*60000).toISOString(),created_at:now(),demo_code:code};
    ...
    const supabase=getSupabaseBrowserClient() as any;const {data,error}=await supabase.from("master_company_access_requests").insert({...row,id:undefined,demo_code:undefined,master_profile_id:profile?.id,code_hash:code}).select().single();if(error){setMessage(error.message);return;}setRequests(v=>[data,...v]);setAccessCompany(null);setMessage("Access request created.");
  }
  ...
  return <main className="master-shell">
    ...
    <section className="master-content">{message&&<div className="master-alert">{message}<button onClick={()=>setMessage("")}>×</button></div>}
      ...
      {tab==="leads"&&<><header className="master-header"><div><span className="master-kicker">LEAD DISTRIBUTION</span><h2>Lead Center</h2><p>Assign a lead, notify the company and convert it into Customer + Property.</p></div><button className="master-primary" onClick={()=>setShowLeadForm(true)}>+ New lead</button></header><div className="master-table-wrap"><table className="master-table"><thead><tr><th>Lead</th><th>Service</th><th>Company</th><th>Status</th><th></th></tr></thead><tbody>{leads.map(l=><tr key={l.id}><td><strong>{l.full_name}</strong><small>{l.address||l.email||l.phone}</small></td><td>{l.service_requested||"—"}</td><td>{companyName(l.assigned_company_id)}</td><td><span className="master-status">{l.status}</span></td><td>{l.status!=="converted"&&<button className="master-inline-button" onClick={()=>convertLead(l)}>Company accepts</button>}</td></tr>)}</tbody></table>{!leads.length&&<div className="master-empty">No leads yet.</div>}</div></>}
