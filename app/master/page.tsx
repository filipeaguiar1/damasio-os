...
import {Estimate,createInvoiceFromEstimate,getEstimates,reviseEstimateTotal,saveEstimate,updateEstimateStatus} from "@/lib/storage";
...
type Company = { id:string; name:string; slug:string; active:boolean; plan_name:string; contact_email?:string; referral_code?:string; stripe_connect_status?:string; stripe_connected_account_id?:string|null; created_at?:string; deleted_at?:string|null; purge_after?:string|null; deletion_reason?:string|null; trash_active_member_ids?:string[] };
type CompanyMember = { id:string; company_id:string; kind:"admin"|"employee"|"customer"; name:string; email?:string; active:boolean };
type Lead = { id:string; full_name:string; email?:string; phone?:string; address?:string; service_requested?:string; notes?:string|null; status:string; assigned_company_id?:string|null; created_at?:string; updated_at?:string };
...
export default function MasterPage(){
  ...
  const[leadForm,setLeadForm]=useState({companyId:"",amount:"",message:"",status:"offered"});
  ...
  async function generatePayoutBatch(){
    ...
    if(isDemo){const total=payoutItems.filter(item=>item.company_id===companyId&&item.status==="eligible").reduce((sum,item)=>sum+Number(item.transfer_amount||0),0);const batch:PayoutBatch={id:uid("batch"),company_id:companyId,week_start:window.weekStart,week_end:window.weekEnd,scheduled_payout_date:window.payoutFriday,status:"draft",total_transfer_amount:total,approved_at:null,created_at:now()};setPayoutBatches(v=>[batch,...v]);setMessage(`Weekly payout draft prepared for ${companyName(companyId)}. Review it before approval.`);return;}
    const token=await getAccessToken();if(!token){setMessage("Your Master login expired. Sign in again.");return;}
    const response=await fetch("/api/master/payouts",{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${token}`},body:JSON.stringify({action:"prepare",companyId})});
  ...
  async function approvePayoutBatch(batch:PayoutBatch){
    if(batch.status!=="draft")return;
    if(isDemo){setPayoutBatches(v=>v.map(item=>item.id===batch.id?{...item,status:"approved",approved_at:now()}:item));setPayoutItems(v=>v.map(item=>item.company_id===batch.company_id&&item.status==="eligible"?{...item,status:"approved",approved_at:now()}:item));setMessage(`Weekly payout approved for Friday ${batch.scheduled_payout_date}.`);return;}
    const token=await getAccessToken();if(!token){setMessage("Your Master login expired. Sign in again.");return;}
    const response=await fetch("/api/master/payouts",{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${token}`},body:JSON.stringify({action:"approve",batchId:batch.id})});
    const result=await response.json();if(!response.ok){setMessage(result.error||"Payout batch could not be approved.");return;}
    setMessage(result.message);await loadPayouts(token);
  }
  ...
  async function createLead(e:FormEvent<HTMLFormElement>){e.preventDefault();const fd=new FormData(e.currentTarget);const row:Lead={id:uid("lead"),full_name:String(fd.get("name")||""),email:String(fd.get("email")||""),phone:String(fd.get("phone")||""),address:String(fd.get("address")||""),service_requested:String(fd.get("service")||""),assigned_company_id:String(fd.get("company")||"")||undefined,status:String(fd.get("company")||"")?"offered":"new",created_at:now()};
    if(isDemo){setLeads(v=>[row,...v]);addAudit("lead.created",row.assigned_company_id||undefined,{lead:row.full_name});setShowLeadForm(false);return;}
  ...
  function openLead(lead:Lead){setSelectedLead(lead);setLeadForm({companyId:lead.assigned_company_id||"",amount:"",message:`Hi ${lead.full_name}, your ${lead.service_requested||"service"} quote has been reviewed.`,status:lead.status==="new"?"offered":lead.status})}
  async function saveLeadResponse(e:FormEvent<HTMLFormElement>){e.preventDefault();if(!selectedLead)return;const amount=Number(leadForm.amount||0);if(!leadForm.companyId){setMessage("Choose the company that will serve this customer.");return;}if(!amount||amount<=0){setMessage("Enter the final quoted amount before sending the response.");return;}const responseNote=[selectedLead.notes,`Master response: ${leadForm.message}`,`Final quoted amount: $${amount.toFixed(2)}`,`Response status: ${leadForm.status}`].filter(Boolean).join(" | ");const patch={assigned_company_id:leadForm.companyId,status:"offered",notes:responseNote,updated_at:now()};
    if(isDemo){const saved=saveEstimate({validUntil:new Date(Date.now()+14*86400000).toISOString().slice(0,10),customer:selectedLead.full_name,phone:selectedLead.phone||"",email:selectedLead.email||"customer@damasioos.demo",address:selectedLead.address||"",title:selectedLead.service_requested||"Property service",description:leadForm.message,status:"sent",items:[{id:uid("item"),type:"service",description:selectedLead.service_requested||"Property service",quantity:1,unit:"service",unitPrice:Math.round((amount/1.13)*100)/100}],notes:responseNote});const invoice=createInvoiceFromEstimate(saved.id);setLeads(v=>v.map(lead=>lead.id===selectedLead.id?{...lead,...patch}:lead));addAudit("lead.response_sent",leadForm.companyId,{lead:selectedLead.full_name,amount,invoice:invoice?.number});setSelectedLead(null);setMessage(`Demo linked: quote ${saved.number} and invoice ${invoice?.number||""} now appear in the customer payment area.`);return;}
    const token=await getAccessToken();if(!token){setMessage("Your Master login expired. Sign in again.");return;}
    const response=await fetch(`/api/master/leads/${selectedLead.id}/respond`,{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${token}`},body:JSON.stringify({companyId:leadForm.companyId,finalTotal:amount,message:leadForm.message,sendInvite:true})});
    const result=await response.json();if(!response.ok){setMessage(result.error||"Response could not be sent.");return;}
    setLeads(v=>v.map(lead=>lead.id===selectedLead.id?{...lead,...patch,status:"offered"}:lead));setSelectedLead(null);setMessage(result.message||"Response sent and customer invoice prepared.");
  }
  ...
  async function startStripeOnboarding(c:Company){
    if(isDemo){setCompanies(v=>v.map(company=>company.id===c.id?{...company,stripe_connect_status:"onboarding",stripe_connected_account_id:"acct_demo"}:company));setSelectedCompany({...c,stripe_connect_status:"onboarding",stripe_connected_account_id:"acct_demo"});setMessage("Demo Stripe onboarding link prepared. In production this opens Stripe Express onboarding.");return;}
    const token=await getAccessToken();if(!token){setMessage("Your Master login expired. Sign in again.");return;}
    const response=await fetch(`/api/master/companies/${c.id}/stripe-onboarding`,{method:"POST",headers:{authorization:`Bearer ${token}`}});
    const result=await response.json();if(!response.ok){setMessage(result.error||"Stripe onboarding could not start.");return;}
  ...
  return <main className="master-shell">
    ...
    <section className="master-content">{message&&<div className="master-alert">{message}<button onClick={()=>setMessage("")}>×</button></div>}
      ...
      {tab==="leads"&&<><header className="master-header"><div><span className="master-kicker">LEAD DISTRIBUTION</span><h2>Lead Center</h2><p>Open a request, assign a company, edit the final value and save the response.</p></div><button className="master-primary" onClick={()=>setShowLeadForm(true)}>+ New lead</button></header><div className="master-table-wrap"><table className="master-table"><thead><tr><th>Lead</th><th>Service</th><th>Company</th><th>Status</th><th>Action</th></tr></thead><tbody>{leads.map(l=><tr key={l.id}><td><strong>{l.full_name}</strong><small>{l.address||l.email||l.phone}</small></td><td>{l.service_requested||"—"}<small>{l.notes||""}</small></td><td>{companyName(l.assigned_company_id)}</td><td><span className="master-status">{l.status}</span></td><td><button className="master-inline-button" onClick={()=>openLead(l)}>Open / edit</button></td></tr>)}</tbody></table>{!leads.length&&<div className="master-empty">No leads yet.</div>}</div></>}
      ...
      {tab==="payouts"&&payoutBatches.some(batch=>(!payoutCompany||batch.company_id===payoutCompany)&&batch.status==="draft")&&<section className="master-season-panel" style={{marginTop:18}}><div className="master-season-save"><span>Draft batches do not transfer money until Master approves them.</span><div className="row">{payoutBatches.filter(batch=>(!payoutCompany||batch.company_id===payoutCompany)&&batch.status==="draft").map(batch=><button key={batch.id} className="master-primary" disabled={Number(batch.total_transfer_amount||0)<0.5} onClick={()=>void approvePayoutBatch(batch)}>Approve {companyName(batch.company_id)} · ${Number(batch.total_transfer_amount||0).toFixed(2)}</button>)}</div></div></section>}
    ...
    {selectedLead&&<Modal title={`Quote response: ${selectedLead.full_name}`} onClose={()=>setSelectedLead(null)}><form className="master-form" onSubmit={saveLeadResponse}><label>Assign company<select value={leadForm.companyId} onChange={e=>setLeadForm(v=>({...v,companyId:e.target.value}))}><option value="">Unassigned</option>{companies.filter(c=>c.active).map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label>Final quoted amount<input type="number" min="0" step="0.01" value={leadForm.amount} onChange={e=>setLeadForm(v=>({...v,amount:e.target.value}))} placeholder="0.00"/></label><label>Response message<textarea value={leadForm.message} onChange={e=>setLeadForm(v=>({...v,message:e.target.value}))}/></label><label>Status<select value={leadForm.status} onChange={e=>setLeadForm(v=>({...v,status:e.target.value}))}><option value="offered">Offered / response ready</option><option value="accepted">Accepted</option><option value="declined">Declined</option><option value="archived">Archived</option></select></label><p className="master-form-note">This creates or links the customer, creates the property, quote and pending invoice, then sends the customer invite when email is available.</p><button>Send response + create customer invite</button></form></Modal>}
