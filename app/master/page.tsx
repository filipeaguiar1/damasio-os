...
export default function MasterPage(){
  ...
  async function saveLeadResponse(e:FormEvent<HTMLFormElement>){e.preventDefault();if(!selectedLead)return;const amount=Number(leadForm.amount||0);const responseNote=[selectedLead.notes,`Master response: ${leadForm.message}`,amount>0?`Final quoted amount: $${amount.toFixed(2)}`:null,`Response status: ${leadForm.status}`].filter(Boolean).join(" | ");const patch={assigned_company_id:leadForm.companyId||null,status:leadForm.status,notes:responseNote,updated_at:now()};
    if(isDemo){setLeads(v=>v.map(lead=>lead.id===selectedLead.id?{...lead,...patch}:lead));addAudit("lead.response_saved",leadForm.companyId||undefined,{lead:selectedLead.full_name,amount});setSelectedLead(null);setMessage("Lead response saved locally. Customer notification/email wiring is the next step.");return;}
    const supabase=getSupabaseBrowserClient() as any;const{data,error}=await supabase.from("lead_center").update(patch).eq("id",selectedLead.id).select().single();if(error){setMessage(error.message);return;}setLeads(v=>v.map(lead=>lead.id===selectedLead.id?data:lead));setSelectedLead(null);setMessage(`Response saved for ${selectedLead.full_name}. Email sending still needs provider configuration.`)
  }
