...
export default function MasterPage(){
  ...
  async function applySeason(nextMode:SeasonMode,nextSeason:Season){setSeasonMode(nextMode);setSeason(nextSeason);try{const config=isDemo?saveLocalSeasonTheme(nextMode,nextSeason):await updateSeasonTheme(nextMode,nextSeason);setMessage(`Platform theme changed to ${config.effectiveSeason}. Every connected device will refresh automatically.`)}catch(error){saveLocalSeasonTheme(nextMode,nextSeason);setMessage(error instanceof Error?`${error.message} Local preview applied; run the season migration for global sync.`:"Local preview applied.")}}
  ...
  return <main className="master-shell">
    ...
    <section className="master-content">{message&&<div className="master-alert">{message}<button onClick={()=>setMessage("")}>×</button></div>}
      ...
      {tab==="theme"&&<><header className="master-header"><div><span className="master-kicker">GLOBAL EXPERIENCE</span><h2>Season Theme</h2><p>One switch changes colours, weather effects and the animated worker on desktop and mobile.</p></div><div className="master-summary"><b>{seasonMode==="auto"?"Auto":season}</b><span>active mode</span></div></header><section className="master-season-panel"><div className="master-season-mode"><button className={seasonMode==="auto"?"active":""} onClick={()=>void applySeason("auto",season)}>Automatic</button><button className={seasonMode==="manual"?"active":""} onClick={()=>void applySeason("manual",season)}>Manual</button></div><p>{seasonMode==="auto"?"Northern hemisphere schedule: Spring Mar–May, Summer Jun–Aug, Autumn Sep–Nov, Winter Dec–Feb.":"The selected season remains active until Master changes it."}</p><div className="master-season-grid">{([['spring','🌸','Spring','Fresh green with falling petals'],['summer','🌿','Summer','Lawn mowing and bright greens'],['autumn','🍂','Autumn','Warm colours and falling leaves'],['winter','❄','Winter','Falling snow and snow worker']] as [Season,string,string,string][]).map(([key,icon,label,description])=><button key={key} className={season===key?"active":""} onClick={()=>void applySeason("manual",key)}><i>{icon}</i><strong>{label}</strong><small>{description}</small></button>)}</div></section></>}
