...
import { DAMASIO_CREWS, DAMASIO_SYNC_EVENT, Lead, dayNameFromDate, getLeads, isLeadAvailableForRoute, scheduleLead, scheduleRouteBatch, seedDemoLeads, unassignHomes } from "@/lib/storage";
...
export default function Schedule(){
  ...
  function assignOne(id:string){
    if(!date){setMessage("Select a service date first.");return;}
    const ok=scheduleLead(id,date,serviceWindow,crew);
    setMessage(ok?"House assigned. It will now appear on Routes and Employee Route for the matching day/crew.":"This house is already assigned to another route. Remove it from that route before assigning again.");
    refresh();
  }
  ...
  function createRoute(){
    ...
    const total=scheduleRouteBatch(selected,date,serviceWindow,crew);
    setMessage(total>0?`${total} house(s) assigned to ${crew} on ${routeDay}. Already assigned houses were skipped/locked.`:"No houses were assigned. The selected houses are already assigned or completed.");
    setSelected([]);
    refresh();
  }
  ...
  return <AdminShell active="Dispatch">
    ...
    <section className="card profile-card" style={{marginBottom:20}}>
      ...
      <div className="form-grid">
        ...
        <div className="field"><label>Employee / Crew</label><select className="input" value={crew} onChange={e=>setCrew(e.target.value)}>{DAMASIO_CREWS.map(c=><option key={c}>{c}</option>)}</select></div>
      </div>
      <div className="row" style={{marginTop:16}}>
        ...
        <button className="btn btn-primary" onClick={createRoute}>Create Route</button>
      </div>
    ...
    <section className="card table-card">
      ...
      <div className="table-wrap"><table><thead><tr><th><input type="checkbox" checked={allSelected} onChange={toggleAll}/></th><th>Customer</th><th>Service</th><th>Next Cut</th><th>Day</th><th>Crew</th><th>Status</th><th>Action</th></tr></thead><tbody>{visible.length===0?<tr><td colSpan={8}>No customers/jobs yet. Load Demo or create customers first.</td></tr>:visible.map(l=>{const canAssign=isLeadAvailableForRoute(l);return <tr key={l.id} style={!canAssign?{opacity:.62}:undefined}><td><input type="checkbox" checked={selected.includes(l.id)} onChange={()=>toggle(l.id)} disabled={!canAssign}/></td><td><strong>{l.name}</strong><br/><small>{l.address}</small>{!canAssign&&<><br/><small>Locked: assigned to {l.assignedCrew||"route"} {l.scheduledDate?`on ${l.scheduledDate}`:""}</small></>}</td><td>{l.service}</td><td>{l.nextVisitDate||l.scheduledDate||"Not scheduled"}</td><td>{l.serviceDay||"-"}</td><td>{l.assignedCrew||"-"}</td><td>{canAssign?"available":l.status}</td><td><div className="row">{canAssign?<button className="btn btn-outline" onClick={()=>assignOne(l.id)} disabled={!date}>Assign</button>:<><span className="btn btn-outline" aria-disabled="true">Assigned</span><button className="route-reset-btn" title="Remove from route and make available again" aria-label={`Reset ${l.name} assignment`} onClick={()=>{if(window.confirm(`Reset ${l.name} assignment? This removes the route, crew and day, then returns the job to Available.`)){unassignHomes([l.id]);setSelected(current=>current.filter(id=>id!==l.id));setMessage(`${l.name} was removed from the route and is Available again.`);refresh()}}}>↻</button></>}</div></td></tr>})}</tbody></table></div>
    </section>
  </AdminShell>
}
