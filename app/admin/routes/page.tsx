...
export default function RoutesPage(){
  ...
  const[propertyPreview,setPropertyPreview]=useState<Lead|null>(null);
  ...
  function changeMode(next:Mode){setMode(next);setSelected([]);setQuery("");setPropertyPreview(null);window.history.replaceState(null,"",`/admin/routes?tab=${next}`)}
  ...
  async function save(action:"assign"|"smart"|"move"){
    ...
    if(action==="smart"&&startMode==="manual"&&!manualStartAddress.trim()){setMessage("Enter the manual starting address before creating the Smart Route.");return}
    if(action==="smart"&&startMode==="employee"&&!destination.routeStartAddress){setMessage("This Employee has no default route address. Choose a manual starting address or update the Employee profile.");return}
    setBusy(true);
    try{
      const token=await accessToken();
      const response=await fetch("/api/admin/routes",{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${token}`},body:JSON.stringify({action,jobIds:selectedJobs.map(item=>item.canonicalJobId||item.id),crewId:destination.crewId,routeDate:action==="move"?targetDate:action==="smart"?date:undefined,startAddress:action==="smart"?(startMode==="employee"?destination.routeStartAddress:manualStartAddress.trim()):undefined})});
      const result=await response.json();
      if(!response.ok)throw new Error(result.error||"The route change could not be saved.");
      setSelected([]);
      setMessage(action==="assign"?`${result.count} customer${result.count===1?"":"s"} assigned to ${destination.name}.`:action==="move"?`${result.count} house${result.count===1?"":"s"} moved to ${destination.name} on ${targetDate}.`:`Smart Route created for ${destination.name} on ${date}.`);
      await refresh();
  ...
  async function returnToAvailable(home:Lead){
    if(!window.confirm(`Return ${home.name} to available customers?`))return;
    setBusy(true);
    try{
      const token=await accessToken();
      const response=await fetch("/api/admin/routes",{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${token}`},body:JSON.stringify({action:"unassign",jobIds:[home.canonicalJobId||home.id]})});
      const result=await response.json();
      if(!response.ok)throw new Error(result.error||"Customer could not be returned to available.");
      setPropertyPreview(null);setMessage(`${home.name} returned to available customers.`);await refresh();
    }catch(error){setMessage(error instanceof Error?error.message:"Customer could not be returned to available.")}
    finally{setBusy(false)}
  }
  ...
  return <AdminShell active="Routes">
    ...
    <style jsx global>{`
      ...
      .route-property-preview{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:15px 17px;border-top:1px solid #dce8e2;background:#fff}.route-property-preview div{display:grid;gap:3px}.route-property-preview span{font-size:10px;font-weight:900;letter-spacing:.12em;text-transform:uppercase;color:#147653}.route-property-preview small,.route-property-preview em{font-size:12px;color:#667970}.route-return-button{display:grid;place-items:center;width:42px;height:42px;border:1px solid #b8d2c5;border-radius:50%;background:#eef8f3;color:#08704d;font-size:22px;font-weight:900;cursor:pointer}
      .smart-route-start-card{display:grid;grid-template-columns:minmax(220px,1fr) repeat(2,minmax(220px,1fr));gap:12px;align-items:stretch;margin-top:18px;padding:18px;border:1px solid #dbe7e1;border-radius:20px;background:#fff}.smart-route-start-card>div{display:grid;align-content:center;gap:4px}.smart-route-start-card>div span{font-size:10px;font-weight:900;letter-spacing:.12em;text-transform:uppercase;color:#147653}.smart-route-start-card label{display:flex;gap:10px;align-items:center;padding:13px;border:1px solid #d8e5df;border-radius:14px;cursor:pointer}.smart-route-start-card label.selected{border-color:#0b7655;background:#eef8f3}.smart-route-start-card label span{display:grid;gap:3px}.smart-route-start-card label small{color:#667970}.smart-route-start-card>div:last-child{grid-column:1/-1}
      ...
      @media(max-width:900px){.route-controls-dynamic,.route-move-row,.smart-route-start-card{grid-template-columns:1fr}.desktop-route-workspace{grid-template-columns:1fr}.desktop-route-modes{overflow-x:auto}.desktop-route-modes button{min-width:120px}}
    `}</style>
  </AdminShell>
}
