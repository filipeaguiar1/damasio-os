...
function uniqueIds(values:(string|null|undefined)[]){return [...new Set(values.filter((value):value is string=>Boolean(value)))]}
...
async function requireEmployee(request:NextRequest){
  ...
  const{data:profile,error:profileError}=await auth.from("profiles").select("id,role,active,company_id,organization_id,email,full_name,avatar_url").eq("id",user.id).maybeSingle();
  ...
  let{data:employee,error:employeeError}=await client.from("employees").select(columns).eq("profile_id",user.id).eq("active",true).or(companyFilter(profileCompanyId)).maybeSingle();
  if(employeeError)throw new Error(employeeError.message);

  if(!employee&&user.email){
    const normalized=user.email.trim().toLowerCase();
    const result=await client.from("employees").select(columns).ilike("email",normalized).eq("active",true).or(companyFilter(profileCompanyId)).order("created_at",{ascending:true}).limit(2);
    if(result.error)throw new Error(result.error.message);
    if((result.data||[]).length>1)throw new Error("More than one active Employee record matches this login. Ask Admin to keep only the canonical record.");
    employee=result.data?.[0]||null;
    if(employee){
      const employeeCompanyId=employee.company_id||employee.organization_id||profileCompanyId;
      const{data:linked,error:linkError}=await client.from("employees").update({profile_id:user.id,email:normalized,company_id:employeeCompanyId,organization_id:employeeCompanyId}).eq("id",employee.id).select(columns).single();
      if(linkError)throw new Error(linkError.message);
      employee=linked;
      const profileUpdate=await client.from("profiles").update({email:normalized,company_id:employeeCompanyId,organization_id:employeeCompanyId}).eq("id",user.id);
  ...
  if(companyId!==profileCompanyId)throw new Error("Employee and profile belong to different companies. Ask Admin to repair the canonical account link.");
  return{client,employee:employee as EmployeeRow,userId:user.id,companyId,avatarUrl:profile.avatar_url||null};
}

async function loadRoute(client:ReturnType<typeof serviceClient>,employee:EmployeeRow,companyId:string,date:string,avatarUrl:string|null){
  ...
  const rows=((visitResult.data||[]) as RouteVisitRow[]).filter(row=>
    row.assigned_employee_id===employee.id||Boolean(!row.assigned_employee_id&&employee.crew_id&&row.crew_id===employee.crew_id)
  );
  ...
  const[propertyResult,customerResult,jobResult,noteResult]=await Promise.all([
    propertyIds.length?client.from("properties").select("id,address_line1,city,province,postal_code,latitude,longitude").in("id",propertyIds).or(companyFilter(companyId)):empty,
    customerIds.length?client.from("customers").select("id,full_name").in("id",customerIds).or(companyFilter(companyId)):empty,
    jobIds.length?client.from("jobs").select("id,service_name").in("id",jobIds).or(companyFilter(companyId)):empty,
    visitIds.length?client.from("activity_log").select("entity_id,details,created_at").eq("organization_id",companyId).eq("entity_type","visit").eq("action","visit.employee_note").in("entity_id",visitIds).order("created_at",{ascending:false}):empty,
  ]);
  ...
  const properties=new Map((propertyResult.data||[]).map((row:any)=>[row.id,row]));
...
export async function PATCH(request:NextRequest){
  try{
    ...
    const{data:visit,error:visitError}=await client.from("visits").select("id,company_id,organization_id,assigned_employee_id,crew_id,status,started_at,scheduled_date").eq("id",body.visitId).or(companyFilter(companyId)).maybeSingle();
    if(visitError||!visit)throw new Error(visitError?.message||"Visit not found.");
    const allowed=visit.assigned_employee_id===employee.id||Boolean(!visit.assigned_employee_id&&employee.crew_id&&visit.crew_id===employee.crew_id);
    ...
    if(body.action==="start"){
      if(["completed","cancelled"].includes(visit.status))throw new Error("This visit can no longer be started.");
    ...
    const{data:updated,error:updateError}=await client.from("visits").update(patch).eq("id",visit.id).or(companyFilter(companyId)).select("id,status,started_at,finished_at,duration_seconds").single();
