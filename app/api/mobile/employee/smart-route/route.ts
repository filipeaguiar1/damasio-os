...
function nearestOrder(count: number, matrix: Array<Array<number | null>>, alternative: number) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.action === "optimize") {
      ...
      const raw = nearestOrder(stops.length, matrix.distances, Math.max(0, Number(body.alternative || 0)));
      const order = twoOpt(raw, matrix.distances);
