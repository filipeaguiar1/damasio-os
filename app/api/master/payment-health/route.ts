...
export async function GET(request: NextRequest) {
  try {
    ...
    const [orgsR,invoicesR,paymentsR,webhooksR,agreementsR,cyclesR,eventsR,payoutsR,balancesR,withdrawalsR,masterBalanceR] = await Promise.all([
      ...
      db.from("invoices").select("id,invoice_number,status,total,created_at,stripe_payment_intent_id,stripe_checkout_session_id,billing_cycle_id,billing_event_id,visit_id,manual_created_by_profile_id").order("created_at", { ascending:false }).limit(5000),
    ...
    const financialInvoices=invoices.filter((i:any)=>!String(i.invoice_number||"").startsWith("SIM-"));
    const simulationInvoices=invoices.length-financialInvoices.length;
    ...
    const paidWithoutPayment=financialInvoices.filter((i:any)=>i.status==="paid"&&!paidByInvoice.has(String(i.id)));
    const mismatches=financialInvoices.filter((i:any)=>i.status==="paid"&&paidByInvoice.has(String(i.id))&&Math.abs(Number(i.total)-Number(paidByInvoice.get(String(i.id)).amount))>.009);
    ...
    const invalidPayoutMath=payouts.filter((p:any)=>p.ownership_type&&!['cancelled','refunded','disputed'].includes(String(p.status))&&(
    ...
    stages.push({key:"invoice",label:"Invoice → Payment",status:paidWithoutPayment.length||mismatches.length?"critical":"healthy",detail:`${financialInvoices.length} financial invoice(s), ${payments.length} payment(s), ${paidWithoutPayment.length} paid invoice(s) without payment ledger; ${simulationInvoices} simulation invoice(s) ignored.`});
    ...
    stages.push({key:"balance",label:"Company receivables ledger",status:impossibleBalance.length||staleTransfers.length||transferredMissingId.length||invalidPayoutMath.length?"critical":oldAvailable.length?"warning":"healthy",detail:`${balances.length} ledger entry(s), ${staleTransfers.length} stale transfer(s), ${invalidPayoutMath.length} active payout math mismatch(es), ${oldAvailable.length} balance(s) near safety-payout threshold.`});
    ...
    if(paidWithoutPayment.length)issues.push({severity:"critical",code:"invoice_without_payment",message:`${paidWithoutPayment.length} paid financial invoice(s) have no matching paid payment record.`});
    ...
    if(invalidPayoutMath.length)issues.push({severity:"critical",code:"company_payout_math",message:`${invalidPayoutMath.length} active company payout item(s) do not reconcile gross entitlement minus actual Stripe fee.`});
