      ...
      for (let dayIndex = 0; dayIndex < dailyGroups.length; dayIndex += 1) {
        ...
        dailyGroups[dayIndex].forEach((chain, routeIndex) => {
          ...
          photos.push({
            ...
            storage_path: `${companyId}/operational-simulation/after.png`,
            public_url: null,
            photo_type: "after",
...
async function removeSimulation(service: any, companyId: string) {
  ...
  const storageDelete = await service.storage.from("work-photos").remove([`${companyId}/operational-simulation/after.png`]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const photoStoragePath = `${companyId}/operational-simulation/after.png`;
