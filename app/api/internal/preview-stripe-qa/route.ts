...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!webhookSecret || !supabaseUrl || !serviceKey) return fail("Stripe or Supabase QA configuration is incomplete.", 503);

    const db = createClient(supabaseUrl, serviceKey, { auth: { persistSession: false, autoRefreshToken: false } }) as any;
    const customerResult = await db.from("customers")
      ...
      .limit(1)
      .maybeSingle();
    if (customerResult.error || !customerResult.data) throw new Error(customerResult.error?.message || "No active simulation Customer was found.");
    ...
    if (!invoiceResult.data) {
      ...
      let inserted = await db.from("invoices").insert(invoice).select("id,invoice_number,status,total").single();
      if (inserted.error && /company_id|schema cache|does not exist/i.test(inserted.error.message)) {
        ...
        inserted = await db.from("invoices").insert(legacy).select("id,invoice_number,status,total").single();
      }
    ...
    const metadata = { invoiceId: invoice.id, companyId, customerId: customer.id, qaMode: "protected_preview" };
    ...
    if (paymentIntent.status !== "succeeded") throw new Error(`Invoice test payment ended as ${paymentIntent.status}.`);
    ...
    if (!webhookResponse.ok) throw new Error(`Canonical webhook failed: ${webhookBody.error || webhookResponse.status}.`);
    ...
    if (walletIntent.status !== "succeeded") throw new Error(`Wallet test payment ended as ${walletIntent.status}.`);
    ...
    const [paidInvoice, payment, payout, walletTransaction] = await Promise.all([
      db.from("invoices").select("id,status,stripe_payment_intent_id").eq("id", invoice.id).maybeSingle(),
      db.from("payments").select("id,status,amount").eq("stripe_payment_intent_id", paymentIntent.id).maybeSingle(),
      db.from("company_payout_items").select("id,status,amount_total").eq("stripe_transfer_group", transferGroup).maybeSingle(),
      db.from("customer_wallet_transactions").select("id,balance_after_cents").eq("stripe_payment_intent_id", walletIntent.id).maybeSingle(),
    ]);
    const error = paidInvoice.error || payment.error || payout.error || walletTransaction.error;
    if (error) throw new Error(error.message);
    if (paidInvoice.data?.status !== "paid" || payment.data?.status !== "paid" || !walletTransaction.data) {
      throw new Error("Canonical Stripe records were not finalized.");
    }
    ...
    return NextResponse.json({
      ...
      commit,
      stripeMode: "test",
      testCard: "pm_card_visa (4242 test equivalent)",
      invoice: paidInvoice.data,
      payment: payment.data,
      payout: payout.data,
      wallet: {
        paymentIntentId: walletIntent.id,
        balanceCredits: Number(walletTransaction.data.balance_after_cents || 0) / 100,
      },
      webhook: { received: Boolean(webhookBody.received), duplicate: Boolean(webhookBody.duplicate), eventId },
