...
type RequestBody = {
  ...
  origin?: { label?: string; latitude?: number | null; longitude?: number | null } | null;
...
function normalizeOrder(values?: string[]) { return (values || []).map(String).filter(Boolean); }
function sameOrder(left: string[], right: string[]) { return left.length === right.length && left.every((value, index) => value === right[index]); }
...
async function projectCanonicalVisitOrder(service: any, routeId: string, source: string) {
  ...
  if (!projection.error) return projection.data || {};

  if (!/sync_canonical_route_stops_v2|schema cache|could not find the function|permission denied/i.test(projection.error.message || "")) {
    throw new Error(projection.error.message || "Canonical Route Visit projection could not be synchronized.");
  }

  console.warn("canonical-route-projection-rpc-unavailable", {
    routeId,
    source,
...
async function requireContext(request: NextRequest) {
  ...
  return { service, user: userClient(token), profile, companyId: String(companyId) };
}

async function requireRouteAccess(context: Awaited<ReturnType<typeof requireContext>>, routeId: string) {
  const routeResult = await context.service.from("routes").select("id,crew_id,company_id,organization_id").eq("id", routeId).maybeSingle();
  if (routeResult.error) throw new Error(routeResult.error.message);
  const route = routeResult.data;
  if (!route || String(route.company_id || route.organization_id) !== context.companyId) throw new Error("Route not found in this company.");

  if (String(context.profile.role) === "employee") {
    const employeeResult = await context.service.from("employees").select("id,crew_id,active").eq("profile_id", context.profile.id).eq("active", true).or(companyFilter(context.companyId)).maybeSingle();
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!Number.isInteger(expectedVersion) || expectedVersion < 1) throw new Error("A reviewed routeVersion is required before saving.");

    const context = await requireContext(request);
    await requireRouteAccess(context, routeId);

    if (body.action === "restore") {
      const restored = await context.user.rpc("restore_canonical_route_order_v2", { p_route_id: routeId, p_expected_version: expectedVersion });
      ...
      return NextResponse.json({ ...result, routeId, routeVersion: Number(result.version || result.routeVersion || 0), restored: Boolean(result.restored) }, { headers: { "Cache-Control": "no-store, max-age=0" } });
    }
    ...
    if (new Set(orderedVisitIds).size !== orderedVisitIds.length) throw new Error("The reviewed Route contains duplicate houses.");
    ...
    if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) throw new Error("A valid canonical Route origin is required.");

    console.info("canonical-route-order-request", { routeId, expectedVersion, orderedVisitIds, origin: { label: originLabel, latitude, longitude }, actorRole: String(context.profile.role) });
    ...
    const applied = await context.service.rpc("apply_canonical_route_order_v2_service", {
      ...
      p_source: ["admin", "manager", "master"].includes(String(context.profile.role)) ? "admin_canonical_route" : "employee_smart_route_global",
    });
    if (applied.error) throw migrationError(applied.error.message);

    const result = applied.data || {};
    const savedOrder = Array.isArray(result.appliedOrder) ? result.appliedOrder.map(String) : Array.isArray(result.applied_order) ? result.applied_order.map(String) : [];
    const routeVersion = Number(result.version || result.routeVersion || 0);
    if (!sameOrder(savedOrder, orderedVisitIds) || !Number.isInteger(routeVersion) || routeVersion < 1) throw new Error("The database did not confirm the exact canonical order and version.");
    ...
    console.info("canonical-route-order-persisted", { routeId, expectedVersion, routeVersion: verified.routeVersion, orderedVisitIds: verified.orderedVisitIds, origin: verified.origin });
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Canonical route order could not be saved." }, { status: 400, headers: { "Cache-Control": "no-store, max-age=0" } });
  }
}
