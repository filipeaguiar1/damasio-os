...
async function validateProperty(db: any, identity: CustomerIdentity, propertyId: string) {
  let result = await db.from("properties")
...
export async function POST(request: NextRequest) {
  try {
    const { service: db, identity: resolvedIdentity } = await requireCustomerPortalIdentity(request);
    if (!resolvedIdentity.companyId) throw new Error("Customer account has no company identity.");
    const identity: CustomerIdentity = {
      profileId: resolvedIdentity.profileId,
      customerId: resolvedIdentity.customerId,
      companyId: resolvedIdentity.companyId,
    };
