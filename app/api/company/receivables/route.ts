...
async function context(request: NextRequest) {
  ...
  if (profileError || !profile?.active || profile.role !== "admin") {
    throw new Error("Only the active Company Admin can view company receivables.");
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [entriesResult, withdrawalsResult] = await Promise.all([
      db.from("company_balance_entries")
        .select("id,company_id,payout_item_id,payment_id,invoice_id,customer_id,visit_id,amount_cents,paid_out_cents,reserved_cents,state,hold_reason,stripe_transfer_id,released_at,created_at")
        .eq("company_id", company.id)
        .order("created_at", { ascending: false })
        .limit(150),
      db.from("company_withdrawals")
        .select("id,amount_cents,status,system_generated,stripe_payout_id,estimated_arrival_at,failure_message,requested_at,processed_at,paid_at")
        .eq("company_id", company.id)
    ...
    let stripeStatus = company.stripe_connect_status || "not_started";
    ...
    const pendingCents = entries.filter((row: any) => ["pending", "hold", "release_ready", "transferring"].includes(String(row.state)))
      .reduce((sum: number, row: any) => sum + outstanding(row), 0);
    const internalAvailableCents = entries.filter((row: any) => row.state === "available")
      .reduce((sum: number, row: any) => sum + outstanding(row), 0);
    const processingCents = withdrawals.filter((row: any) => ["reserved", "processing"].includes(String(row.status)))
      .reduce((sum: number, row: any) => sum + Number(row.amount_cents || 0), 0);
    const paidOutCents = withdrawals.filter((row: any) => row.status === "paid")
      .reduce((sum: number, row: any) => sum + Number(row.amount_cents || 0), 0);
    const withdrawableCents = Math.max(0, Math.min(internalAvailableCents, stripeAvailableCents));

    const ledger = entries.map((row: any) => ({
      ...row,
      customerName: customerNames.get(String(row.customer_id || "")) || "Customer",
      outstandingCents: outstanding(row),
    }));
    ...
    return NextResponse.json({
      ...
      stripe: {
        status: stripeStatus,
        payoutSchedule: company.stripe_payout_schedule || null,
        availableCents: stripeAvailableCents,
        pendingCents: stripePendingCents,
        error: stripeError,
      },
      balances: {
        pendingCents,
