...
function cadAvailable(balance: Stripe.Balance) { return balance.available.filter((row) => row.currency.toLowerCase() === "cad").reduce((sum, row) => sum + Number(row.amount || 0), 0); }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (amountCents > stripeAvailableCents) return fail("The requested amount is greater than Stripe's currently available CAD balance.", 409);

    const reserved = await db.rpc("reserve_company_withdrawal", { p_company_id: company.id, p_amount_cents: amountCents, p_requested_by: profile.id, p_system_generated: false });
    ...
    if (payout.status === "paid") {
      const complete = await db.rpc("complete_company_withdrawal", { p_withdrawal_id: reservedWithdrawalId, p_stripe_payout_id: payout.id });
