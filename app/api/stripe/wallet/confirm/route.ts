...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!url || !serviceKey || !stripeKey) return failure("Wallet confirmation is not configured yet.", 503);
    ...
    if (session.metadata?.paymentKind !== "wallet_topup") return failure("Stripe session is not a wallet top-up.", 400);
    ...
    if (!customerId || profileId !== auth.user.id) return failure("Wallet top-up does not belong to this customer.", 403);
    if (!Number.isSafeInteger(amountCents) || amountCents < 100 || session.amount_total !== amountCents) return failure("Wallet top-up amount is invalid.", 400);
    ...
    if (ownership.error || !ownership.data || (ownership.data.profile_id !== auth.user.id && String(ownership.data.email || "").toLowerCase() !== String(auth.user.email || "").toLowerCase())) {
      return failure("Wallet top-up customer could not be verified.", 403);
    }
    ...
    return NextResponse.json({ confirmed: true, credited: Boolean(result?.credited), balanceCredits: Number(result?.balance_cents || 0) / 100 });
  } catch (error) {
    console.error("Stripe wallet confirmation failed", error);
    return failure(error instanceof Error ? error.message : "Could not confirm wallet credits.", 500);
  }
}
