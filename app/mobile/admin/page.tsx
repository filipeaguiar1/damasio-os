...
export default function MobileAdminApp(){
 ...
 async function refresh(){try{setData(mapOperations(await loadDailyOperations()));setError("")}catch(nextError){setData(EMPTY_DATA);setError(nextError instanceof Error?nextError.message:"Live operations are temporarily unavailable.")}finally{setLoading(false)}}
 useEffect(()=>{void refresh();const timer=window.setInterval(()=>void refresh(),15000);return()=>window.clearInterval(timer)},[]);
 const actions=[
  {href:"/mobile/admin/command",icon:"⌁",label:"Today",detail:"Team & route progress"},
 ...
 <section className="role-mobile-section role-attention-section"><div className="role-mobile-section-head"><div><span>PRIORITIES</span><h2>Needs attention</h2></div><Link href="/mobile/admin/tasks">All tasks</Link></div>{data.tasks.length?data.tasks.map(task=><Link className="role-mobile-priority" href="/mobile/admin/tasks" key={task.id}><i>!</i><span><strong>{task.title}</strong><small>{task.customer} · {task.address}</small></span><b>›</b></Link>):<div className="role-mobile-clear"><i>✓</i><span><strong>No return visits open</strong><small>Your priority list is clear.</small></span></div>}</section>
