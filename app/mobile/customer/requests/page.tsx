...
export default function MobileCustomerRequests(){
  ...
  async function submit(){setBusy(true);try{if(service==="Return Visit"&&usesLiveTaskBackend()){if(!board.property?.propertyId)throw new Error("Customer property is not connected.");if(!details.trim())throw new Error("Describe what needs another look for a Return Visit.");const taskId=await createLiveCustomerTask({propertyId:board.property.propertyId,title:"Customer Return Visit",issue:details.trim(),priority:"urgent"});if(evidence.length)await uploadLiveTaskPhotos(taskId,evidence,"issue");setMessage("Return Visit sent to Admin with the attached evidence.")}else{const updated=await addCustomerServiceRequest({serviceName:service,message:details});setBoard(updated);setMessage("Request sent to the company.")}setDetails("");setEvidence([]);setOpen(false);refresh()}catch(error){setMessage(error instanceof Error?error.message:"Request could not be sent.")}finally{setBusy(false)}}
  ...
  return <MobileRoleGuard allowed={["customer"]}><main className="mobile-app-shell role-mobile-shell mobile-customer-subpage">
    ...
    <section className="customer-native-hero request"><span>SELF-SERVICE</span><h1>What does your property need?</h1><p>{completeAddress}</p></section>
